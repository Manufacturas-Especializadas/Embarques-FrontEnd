...
interface Props {
    ...
    onEdit?: (flete: Flete) => void;
}

export const FletesTable = ({ data, onDeleteSuccess, onEdit }: Props) => {
    ...
    const handleDelete = async () => {
        if (!deleteModal.flete) return;

        startDeleting();

        try {
            await fletesService.delete(deleteModal.flete.id);
            ...
            console.error("Error al eliminar el flete:", error);
        } finally {
            stopDeleting();
        }
    };
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {currentData.map((flete) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm font-semibold text-blue-600">
                                            {formatCurrency(flete.costOfStay)}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <RoleGuard allowedRoles={["Admin"]}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <button
                                                onClick={() => openDeleteModal(flete)}
                                                className="text-red-600 hover:text-red-900 transition-colors duration-200
                                                    hover:cursor-pointer disabled:text-red-300 disabled:cursor-not-allowed"
                                            >
                                                Eliminar
                                            </button>
                                        </td>
                ...
                {data.length === 0 && (
                    <div className="px-6 py-12 text-center">
                        ...
                        <div className="text-gray-400 text-sm mt-2">
                            No se encontraron fletes para mostrar
                        </div>
                    </div>
                )}
            </div>
